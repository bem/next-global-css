...
function patchWebpackStyleRules(rule, options) {
  ...
  } else if (options.globalScss && rule.test && rule.test.source === regexScssGlobal.source) {
    delete rule.issuer;
  }
}

function withGlobalCss(options = {}) {
  return (nextConfig) => {
    const config = {
      webpack: (config, { isServer }) => {
        if (isServer) {
          ...
          patchClientWebpackConfig(config, options)
        }
        return config
      },
    }
